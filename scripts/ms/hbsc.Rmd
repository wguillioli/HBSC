...
write_csv(d, 
          paste0(dat_fldr, "processed/d.csv"))
...
# Data Type Coercions & Potential for Prediction
...
# set.seed(67)
# data_split <- initial_split(d, strata = mental_issue)
# train_data <- training(data_split)
# test_data  <- testing(data_split)
# 
# # Create cross-validation folds
# cv_folds <- vfold_cv(train_data, v = 10, strata = mental_issue)
# 
...
# 
# lr_recipe <- recipe(mental_issue ~ pmsu_lmh + MBMI, data = train_data) |>
...
# predictions_class <- predict(final_fit, new_data = test_data)
# predictions_class
# 
...
# data split
set.seed(67)
data_split <- initial_split(d, prop = 0.8, strata = mental_issue)
train_data <- training(data_split)
test_data  <- testing(data_split)
...
X_explain <- d[1:100, ] %>% select(-mental_issue)

require(kernelshap)
require(shapviz)

# 2. Compute the SHAP valuese
# Type = "prob" ensures R looks at the predicted probabilities (e.g., .pred_Yes)
shap_results <- kernelshap(
  object = lr_fit, 
  X = X_explain, 
  bg_X = d[1:100, ] %>% select(-mental_issue), # Small background baseline
  type = "prob"
)
...
sv_dependence(shp_viz$.pred_1, v = "MBMI")
...
# Fit Classification following DC
https://campus.datacamp.com/courses/modeling-with-tidymodels-in-r/classification-models?ex=1

```{r}

#data resampling
hbsc_split <- initial_split(d, prop = .8, strata = mental_issue)
d_training <- hbsc_split %>% training()
d_test <- hbsc_split %>% testing()
nrow(d_training)
nrow(d_test)
...
logistic_fit <- logistic_model %>% 
  fit(mental_issue ~ pmsu_lmh,
      data = d_training)

logistic_fit

# Predict 
class_preds <- predict(logistic_fit, new_data = d_test, type = 'class')
prob_preds <- predict(logistic_fit, new_data = d_test, type = 'prob')
...
metrics(results, truth = mental_issue, estimate = .pred_class)
...
last_fit <- logistic_model %>% 
  last_fit(mental_issue ~ pmsu_lmh,
           split = hbsc_split)
...
logistic_fit %>% 
  collect_predictions() %>% 
  # Plot ROC curve
  roc_curve(truth = lifesat_low, .pred_0) %>% 
  autoplot()


```

```{r sandox, include=FALSE}

# lifesat_low baseline
d %>%
